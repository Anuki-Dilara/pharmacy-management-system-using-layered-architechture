...
public class OrderViewFormController implements Initializable {
    ...
    private CustomerBO customerBO= BOFactory.getInstance().getBO(BOFactory.BOTypes.CUSTOMER);
    ...
    private void loadCustomerids() {
        try {
            ...
            List<String> customerids = customerBO.getIDs();
    ...
    void btnCustomerPlaceOrderOnAction(ActionEvent event) {
        ...
        for (int i = 0; i < tblCustomerOrder.getItems().size(); i++) {
            ...
            cartDetails.add(new Cart(tm.getMedicine_id(), tm.getMedicine_name(), tm.getCustomerOrder_quantity(), tm.getCustomerOrder_unitPrice(), tm.getCustomerOrder_amount()));
        }
        System.out.println("For Loop Done");
        PlaceOrder placeOrder = new PlaceOrder(oid,total,date,customerid, cartDetails);
        try {
            boolean placeOrder1 = placeOrderBO.PlaceOrder(new PlaceOrderDTO(placeOrder.getOrder_id(),
                    placeOrder.getOrder_amount(), placeOrder.getOrder_date(), placeOrder.getCustomer_id(), placeOrder.getOrderdetail()));
   ...
   private void calculateNetTotal() {
    ...
    public void cmbCustomerIdOnAction(ActionEvent actionEvent) {
        ...
        try {
            ...
            lblCustomerName.setText(customerDTO.getCustomer_name());
        } catch (SQLException | ClassNotFoundException e) {
            e.printStackTrace();
            new Alert(Alert.AlertType.ERROR, "SQL Error!").show();
        }
